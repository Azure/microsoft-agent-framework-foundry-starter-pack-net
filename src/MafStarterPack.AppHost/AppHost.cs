...
reb.Append($"Endpoint={config["FOUNDRY_PROJECT_ENDPOINT"] ?? throw new InvalidOperationException("Missing Foundry Endpoint")}");
reb.Append($";Deployment={config["FOUNDRY_MODEL_DEPLOYMENT_NAME"] ?? throw new InvalidOperationException("Missing Foundry Model Deployment Name")}");
reb.Append($";AgentName={config["TODO_AGENT_NAME"] ?? throw new InvalidOperationException("Missing Foundry Agent Name")}");
reb.Append($";AgentVersion={config["TODO_AGENT_VERSION"] ?? throw new InvalidOperationException("Missing Foundry Agent Version")}");

var foundry = builder.AddConnectionString("foundry", reb.Build());
